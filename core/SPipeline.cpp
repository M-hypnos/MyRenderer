...
void SPipeline::drawMesh(Mesh& mesh) {
	...
	if (rendererMode_ == RendererMode::Line) {
		for (int i = 0; i < mesh.Indices.size(); i += 3) {
			...
			for (int j = 0; j < 3; j++) {
				Vec4f clip_coords = shader_->vertexShader(mesh.Vertices[mesh.Indices[i+j]]);
				vertIn.emplace_back(clip_coords);

				if (!isClipTriangle_ && isOutsidePlane(clip_coords))
				{
					isNeedClip = true;
					break;
				}
		...
		for (int i = 0; i < mesh.Indices.size(); i += 3) {
			...
			bool isNeedClip = false;
			for (int j = 0; j < 3; j++) {
				Vec4f clip_coords = shader_->vertexShader(tri[j]);
				vertIn.emplace_back(clip_coords, tri[j].normal, tri[j].texcoords);
				if (!isClipTriangle_ && isOutsidePlane(clip_coords))
				{
					isNeedClip = true;
					break;
