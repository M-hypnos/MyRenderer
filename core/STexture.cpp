...
bool STexture::loadFromImage(const char* filename, const char* type) {
...
SColor STexture::getPixel(float u, float v) {
	if (u < 0 || u > 1 || v < 0 || v > 1) return SColor(0);
	return getPixel(int(u * surf_->w), int(v * surf_->h));
}

SColor STexture::getPixel(int x, int y) {
